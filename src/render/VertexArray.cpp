...
VertexArray::VertexArray(const float* vertices, size_t size) { initialize(vertices, size); }
...
void VertexArray::initialize(const float* vertices, size_t size) {
    ...
    // Assuming the vertex data includes positions (3 floats per vertex) and texture coordinates (2
    // floats per vertex)
...
void VertexArray::initializeObj(const std::vector<float>& vertices,
...
VertexArray::~VertexArray() { deleteBuffers(); }
...
void VertexArray::bind() { glBindVertexArray(_vao); }
...
void VertexArray::createElementBuffer(const std::vector<int>& indexData) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, indexData.size() * sizeof(int), indexData.data(),
                 GL_STATIC_DRAW);
}